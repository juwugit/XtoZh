...
\end{align}
Note that $M_{0}$ represents the real mass eigenvalue of the neutral heavy vector boson. Moreover, since we assume $\hat{m}_{V}>\hat{m}_{Z}$, the only variable controls the sign of the tangent function is $c_{H}$, which is model dependent. Once the sign of $c_{H}$ is determined, the tangent can be uniquely inverted.
...
\end{align}
For model B$_{g_{V}=3}$ the dominant BRs are into di-bosons and the fermionic decays are extremely suppressed. Moreover, the total width increases with increasing $g_{V}$ since it is dominated by the di-boson width which grows with $g_{V}$ as expected from Eq.~({\ref{eq:DeterB}}). This model B is particularly interesting for the present search, since it predicts signal cross sections of the order of fb\cite{HVT} \cite{CMS_AN_2015-186} [Fig.~\ref{fig:xsec}], branching ratios to vector bosons close to unity, and thus being accessible at the LHC.

In the latter chapters, the mass eigenstate of the neutral heavy vector boson in model B scenario refers to the $Z'$ particle. Theoretically, $Z'$ particle is from the mechanism of new broken gauge symmetry, it is named after the SM Z boson which is similarly from the mechanism of the broken U(1) gauge symmetry. In the minimal composite Higgs model, the broken SO(5) symmetry is not a gauge symmetry. However, to be consistent with other similar analyses\cite{CMS_AN_2015-186}, we keep this name as our search target in this thesis.\\
\newpage
\begin{figure}[hbtp]
  \begin{center}
